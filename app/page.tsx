...
export default function Home() {
  ...
  const docTypes = ["Birth Certificate", "Marriage Certificate", "Academic Record", "Legal Contract", "Business Document", "Other"];
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // New Fields Merged into the API Call
    ...
    xhr.onload = async () => {
      if (xhr.status === 200) {
        ...
        if (response.stripeUrl) {
          window.location.href = response.stripeUrl; // STRIPE REDIRECT KEPT
        } else {
  ...
  return (
    <main data-theme={theme} className="min-h-screen bg-slate-50 text-slate-800 relative">
      ...
      {/* HEADER: ORIGINAL PADDING TRANSITION KEPT */}
      <header className={`bg-slate-900 border-b border-slate-700 sticky top-0 z-40 transition-all ${theme === 'default' ? 'py-10' : 'py-4'}`}>
        <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
          ...
          <button onClick={handleUploadClick} className="bg-[var(--accent)] hover:bg-[var(--cta-hover)] transition text-white px-5 py-2 rounded-md text-sm font-semibold">Get Started</button>
        </div>
      </header>
      ...
      <section className={`${isLightHero ? "bg-white" : "bg-slate-900"} py-24 border-b border-slate-100 relative z-10`}>
        <div className="max-w-7xl mx-auto px-6 grid md:grid-cols-2 gap-16 items-center">
          <div>
            ...
            <span className="inline-block bg-green-100 text-green-700 text-xs font-bold px-3 py-1 rounded-full mb-6">Trusted by 10,000+ Clients Worldwide</span>
            ...
            {/* EXPANDED FORM WITH ORIGINAL FIELDS */}
            <div className="bg-slate-50 p-6 rounded-2xl border border-slate-200 shadow-sm space-y-3 max-w-md">
              <input type="text" placeholder="Full Name" value={fullName} onChange={(e) => setFullName(e.target.value)} className="w-full p-3 bg-white border border-slate-200 rounded-lg outline-none focus:ring-2 focus:ring-[var(--accent)] text-sm" />
              <div className="grid grid-cols-2 gap-3">
                <input type="email" placeholder="Email" value={email} onChange={(e) => setEmail(e.target.value)} className="p-3 bg-white border border-slate-200 rounded-lg outline-none text-sm" />
                <input type="tel" placeholder="Phone" value={phone} onChange={(e) => setPhone(e.target.value)} className="p-3 bg-white border border-slate-200 rounded-lg outline-none text-sm" />
              </div>
              
              <div className="grid grid-cols-2 gap-3">
                <input type="text" placeholder="Language From" value={langFrom} onChange={(e) => setLangFrom(e.target.value)} className="p-3 bg-white border border-slate-200 rounded-lg text-sm" />
                <input type="text" placeholder="Language To" value={langTo} onChange={(e) => setLangTo(e.target.value)} className="p-3 bg-white border border-slate-200 rounded-lg text-sm" />
              </div>

              <div className="space-y-1">
                <label className="text-[10px] font-bold text-slate-400 uppercase flex items-center">Document Type <HoverNote text="Select document type. Max 10MB." /></label>
                <select value={documentType} onChange={(e) => setDocumentType(e.target.value)} className="w-full p-3 bg-white border border-slate-200 rounded-lg text-xs font-semibold">
                  <option value="">Select Document Type...</option>
                  {docTypes.map(t => <option key={t} value={t}>{t}</option>)}
                </select>
                {documentType === "Other" && (
                  <input type="text" placeholder="Specify Type" value={documentTypeOther} onChange={(e) => setDocumentTypeOther(e.target.value)} className="w-full mt-1 p-3 border-2 border-[var(--accent)] rounded-lg text-sm" />
                )}
              </div>

              <input type="text" placeholder="Postal Address (Optional)" value={address} onChange={(e) => setAddress(e.target.value)} className="w-full p-3 bg-white border border-slate-200 rounded-lg outline-none text-sm" />

              <div className="grid grid-cols-2 gap-3">
                <select value={serviceLevel} onChange={(e) => setServiceLevel(e.target.value)} className="p-3 bg-white border border-slate-200 rounded-lg text-xs font-semibold">
                ...
                <select value={urgency} onChange={(e) => setUrgency(e.target.value)} className="p-3 bg-white border border-slate-200 rounded-lg text-xs font-semibold">
              ...
              <div className="flex justify-between py-1">
                <div className="space-y-1">
                  <label className="text-[10px] font-bold text-slate-400 uppercase flex items-center">Apostille? <HoverNote text="Check if you need legalisation." /></label>
                  <div className="flex gap-3 text-xs">
                    <label><input type="radio" checked={needsApostille} onChange={() => setNeedsApostille(true)} /> Yes</label>
                    <label><input type="radio" checked={!needsApostille} onChange={() => setNeedsApostille(false)} /> No</label>
                  </div>
                </div>
                <div className="space-y-1">
                  <label className="text-[10px] font-bold text-slate-400 uppercase">Physical Copy?</label>
                  <div className="flex gap-3 text-xs">
                    <label><input type="radio" checked={needsPhysical} onChange={() => setNeedsPhysical(true)} /> Yes</label>
                    <label><input type="radio" checked={!needsPhysical} onChange={() => setNeedsPhysical(false)} /> No</label>
                  </div>
                </div>
              </div>

              <button onClick={handleUploadClick} className="w-full bg-[var(--accent)] hover:bg-[var(--cta-hover)] transition text-white py-4 rounded-lg font-bold shadow-lg shadow-[var(--accent)]/20">Upload & Submit Order</button>
            </div>
          </div>

          {/* RIGHT SIDE ASSET REPLACED WITH CLIENT IMAGE */}
          <div className="bg-slate-100 rounded-3xl p-6 text-slate-800 flex items-center justify-center overflow-hidden">
            <img src="/social-media-banner.jpeg" alt="Accucert Trusted" className="rounded-2xl shadow-xl w-full h-auto object-cover" />
          </div>
        </div>
      </section>

      {/* SECTION 2: SERVICES - ORIGINAL SVG PATHS RESTORED */}
      <section id="services" className="bg-white py-24 relative z-10">
        <div className="max-w-7xl mx-auto px-6 text-center">
          ...
          <h2 className="text-4xl font-bold mb-4">Documents We <span className="text-[var(--primary)]">Translate</span></h2>
          <p className="text-slate-600 max-w-2xl mx-auto mb-16">Specialising in official and legal documents accepted worldwide.</p>
          <div className="grid md:grid-cols-3 gap-8 text-left">
            {[
              ...
              <div key={item.t} className="bg-gradient-to-br from-[var(--accent)]/10 to-white rounded-xl p-8 border border-slate-200 text-left hover:border-[var(--accent)] transition-all group">
                <div className="w-10 h-10 mb-4 text-[var(--accent)]"><svg fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2">{item.i}</svg></div>
      ...
      {/* SECTION 3: HOW IT WORKS KEPT */}
      <section id="how-it-works" className="py-24 bg-slate-50 relative z-10">
        <div className="max-w-7xl mx-auto px-6 text-center">
          <h2 className="text-4xl font-bold mb-16">How It <span className="text-[var(--primary)]">Works</span></h2>
      ...
      <footer className="bg-slate-900 text-slate-300 border-t border-slate-700 relative z-10">
        ...
        <div className="text-center text-xs text-slate-500 py-8 border-t border-slate-800">© 2026 Accucert. All rights reserved.</div>
      </footer>
    </main>
  );
}